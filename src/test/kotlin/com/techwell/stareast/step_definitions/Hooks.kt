...
import com.techwell.stareast.driverutil.DriverType
import com.techwell.stareast.driverutil.WebDriverSessionStore
import com.techwell.stareast.driverutil.isMobile
      ...
      if (jobname != null) {
         log.debug(
            "${fillchar.toString().padEnd(debuglength, fillchar)}\n" +
                  ...
                  "${String.format("BUILD_NUMBER: %s", System.getProperty("BUILD_NUMBER")).padEnd(debuglength, fillchar)}\n" +
   ...
   fun afterScenario(scenario: Scenario) {
      ...
      if (webDriverSession?.currentPage != null) {
         try {
            ...
            scenario.log("Debuginfos:\n ${testDataContainer.getAs<List<String>>("debug")}")
            ...
            testDataContainer.getScreenshots().forEachIndexed { index, screenshot ->
               scenario.attach(
                  ...
                  "image/png",
                  "Forced Screenshot - ${index + 1} - ${screenshot.second} "
               )
            }
   ...
   fun a11y(scenario: Scenario) {
      testDataContainer.getAndClearA11Ydescriptions().forEachIndexed { index, issue ->
         scenario.attach(
            issue,
            "text/plain",
            "A11Y Issue ${index + 1} in stepnumber ${testDataContainer.getStepIndex()} "
         )
      }
   }
